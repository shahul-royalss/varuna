...
| 8 | Route, reachability, alerts, pumps | 85 % | 2026-09-10 | **Route is real** (P8.1-P8.6). Time-dependent Dijkstra on the same 21,296 segments the map draws, costed at the depth the vehicle meets when it *arrives*: KEM to Sion plans in **85 ms** against a 300 ms budget, with two alternates and the streets it stepped around named. Eleven tests on a synthetic diamond network - the detour, the naive route walking into the water, arrival-time evaluation, FIFO, the slowdown curve, isochrone monotonicity, the collapse threshold. Two silent defects on the way: `segments.parquet` is EPSG:32643, so a first version compared UTM metres against degrees and snapped every request to one junction; and `class` is a Python keyword, so `itertuples` never carried it and every road took the default speed (ADR-0028). Reachability serves 5/10/15-minute isochrones with the dry sweep cached, and the console's tab draws the rings. The road-conditions feed lists 586 segments impassable for a car. Alerts (P8.7) and pumps (P8.9, P8.10) landed earlier. **Outstanding:** the `/alerts` page's real sender (P8.8, P2), the console's alert and pump tab mirrors (P8.11), and the two motions - the route draw-on (M14) and the isochrone morph (M15). |
