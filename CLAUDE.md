...
- [x] P6.9 `SegmentPopover` on click; hover tooltip ≤ 80 ms (2026-09-11 — the tooltip is deck's own, so there is no React render in its path; the popover gives depth now, peak with its time, and safe-until for all five profiles against the same thresholds the router costs on)
