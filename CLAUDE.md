...
- [x] P3.8 Console: radar animation layer and the fan chart at Hindmata read the cube (temporary panel until Phase 6 wires it properly) (2026-09-09, 3839df5 — `GET /v1/nowcast/rain` and `/rain/series` under the CLAUDE.md 12 URL shape (ADR-0016); `FanChart` is the section 6.6 component Phase 6 keeps, `SkyPanel` is the scaffolding it deletes. Hindmata's position and its `source_url` come from the city register, never a literal. `run_id` is null on a computed cycle because minting one would claim Twin and Flash versions that never ran)
