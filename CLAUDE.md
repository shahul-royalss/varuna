...
| 3 | VARUNA-Sky | 90 % | 2026-09-12 | **Not 100 %: the exit budget is missed and a section 11.1 test fails.** Seven of eight tasks done; `run_sky()` runs QC -> Z-R -> merge -> flow -> STEPS -> products. Timing **6.32 s warm / 8.39 s cold** at 20 x 36 on 120 x 120 against the 5 s budget (the board had carried 5.65 s); pySTEPS is 5.7-6.0 s of it. **ADR-0040, re-measured:** 11.1's third test had never been written. Written first on the shipped fixture it looked catastrophic - the cell flattening 36 -> 12 mm/h - but that is the *fixture*: one Gaussian on a uniform background makes pySTEPS' cascade correlations pure noise. On the bundle's own frames the peak is held (70.5 -> 68.6 mm/h) and the exceedances with it. The **mass** bias is real though, and worse than first reported: **median 21.7 % low at lead 0 across five storms** (6.1-27.3 %), every member losing it individually. `probmatching='mean'` passes the test (+13.2 %) and was **rejected** - it inflates the 99.9th percentile by 52 %, a phantom cloudburst. Ships `xfail(strict=True)`. Earlier fixes hold: fallback AR(2) ringing (ADR-0015), the false Z-R fallback label (ADR-0017), and the Twin being fed the ensemble *median* instead of the mean. P2.9 (IMD decoder) stays P1. |
| 4 | VARUNA-Twin + drains + coupling | 80 % | 2026-09-23 | **P4.5's mass-balance assertion no longer holds on the runs that ship.** The 2026-09-23 re-bake of all seven demo cycles measures **0.127, 0.535, 0.145, 0.260, 0.306, 0.204 and 0.310 %** against section 11.3's 0.1 % budget - every cycle over, 06:40 by more than five times. The board carried 6.1e-04 from a single coupled run, and the previous bake read 0.014-0.091 % on six cycles with 06:40 at 0.215 %, so most cycles have got **worse**, not better. These runs are the first to combine the ADR-0055 tide-datum conversion, the ADR-0039 hotspot fix, the ADR-0044 ES-MDA update and the reversed-edge geometry join; none of those four ran together before, and the tide conversion is the first suspect because it moved the boundary stage 2.7 m and the coupled balance divides by this run's own inflow. **Not diagnosed yet**; the previous runs are backed up, so this is reversible. The earlier figure, for the record: a single coupled run measured **6.1e-04**, inside the budget. **P4.6: 115 s to 74 s, and still missed.** The cost was never the 2D solver - already Numba at 15 s - but the two NumPy stages around it: `drain1d.step` at 106 s and `coupling.compute_exchange` at 25 s, both allocating thirty 50k-element temporaries per call. Both are compiled kernels now (29 s and 4.6 s), tested against the NumPy versions they replace on flap gates, a tide-locked outfall, pressurised nodes and a supply-limited node. The remaining factor of seven needs the edge scatter parallelised by colouring (ADR-0035). Two silent defects fixed on the way: `ExchangeResult` returned shared buffers so two results aliased, and both kernel caches were keyed on `id()`, which CPython reuses - a freed network's arrays reached a longer one and Numba, which does not bounds-check, scattered past the end of a buffer. P4.8 (5 m nests) and P4.9 (PySWMM) stay P1. **2026-09-15:** the Twin can resume from a `TwinState` and returns its final state (ADR-0056, 0535152): 6 steps plus a 6-step resume equal 12 bitwise, the cold path is byte-identical, and the coupled mass balance divides by this run's own inflow, so carried water cannot hide an error. **Nothing in the cycle uses it yet.** The tide boundary is now converted to the DEM's frame (ADR-0055), and nobody has measured reversed flow with it; the drain-graph fix in flight places no Mahim outfall, where the only stored tidal reversed edge was, so the exit's reversed-flow edge must be re-measured on the regraded graph before it is claimed. |
...
| 7 | Pulse, Flash-lite, drain X-ray, what-if | 58 % | 2026-09-23 | **Two of the five open tasks are blocked, not merely unbuilt, and the block is measured.** P7.7 (attribution) and P7.11 (the drawer's attribution and its deep link) cannot be done on Flash-lite: ADR-0042 measured that cleaning **all 21,296 segments** moves the deepest street 3.466 cm (mean 0.1416 cm, against an `ATTRIBUTION_FLOOR_CM` of 0.1), and that cleaning every pipe except one target leaves that target at 71.5821 cm against a base of 71.5821 cm. `simulate()` is element-wise per segment and `baseline_cm` - the tide and the upstream routing, where most of the depth comes from - does not depend on beta at all. Attribution needs `drain1d` inside the loop or P7.12's GNN; there is no third way, and a 50-member P7.6 ensemble built from beta draws would inherit the same insensitivity and report spread it does not have. P7.8's `/v1/whatif` is real but its `/v1/whatif/physics-check` is still a stub in `routers/stubs.py`, and a Mumbai Twin run is 58-114 s against that endpoint's 10 s budget. **Everything below this sentence predates 2026-09-23.** **Pulse is real and works** (P7.1-P7.4): traffic anomalies with confounder rejection, citizen reports on the depth chips, an EnKF over logit(beta) localised by hydraulic hop distance, and  + the desilting CSV. It passes the spec acceptance test - two blocked pipes hidden in a 200-pipe network, twenty observations, both recovered into the top five with their spread cut 40 % - and across the seven pre-ensemble baked cycles the worst pipe read 0.394 to 0.683. The 2026-09-13 re-bake reads 0.451 at 06:10 and 0.622 at 09:10 but not as a climb - 0.620 already at 06:40 - because each cycle re-analyses every observation from the prior rather than carrying the posterior forward (P7.3).  renders it with a live before/after toggle. **Flash-lite is fitted and honest rather than good** (P7.5): 62 ms what-if, 126 ms attribution, both far inside budget, but RMSE 5.7 cm and **CSI 0.085 at 30 cm** on held-out storms - a local-rain cascade cannot reproduce depth that arrives as tide and upstream routing (ADR-0025). So  levels on the Twin and uses the emulator only for the delta, a tide offset is refused rather than approximated, and the measured skill is printed beside every answer. **P7.6 (50-member ensemble products), P7.7 (attribution in the drawer) and P7.12 (GNN) are not claimed**. The seven demo cycles were re-baked on 2026-09-13 with a **20-member street ensemble** (Sky members through Flash-lite, exceedance probabilities in `segments_wet.json`) and deployed; P7.6 stays open because the Pulse parameter draws that make 50 are not built; the physics check (P7.8) exists as an endpoint contract but a Mumbai Twin run is 58-114 s in six of the seven re-baked cycles (47 s in the lightest) against its 10 s budget. **2026-09-15: traffic detection is vectorised and Pulse's static city joins are cached** (ADR-0054, 98b80b8), with identical observations and posteriors at all seven cycles. At 09:10 `run_pulse` is 1.47-1.65 s warm with 10 python processes and 3.56-4.97 s warm with 19-21, and 6.18 s cold with 19-21: the 3 s budget is met warm and missed cold. |
...
- [ ] R3 Twin runs coupled 1D–2D on the city grid with visible surcharge at manholes and reversed flow at a tide-locked outfall. (**The data exists for the first time**, 2026-09-23: the re-bake stores 500 reversed edges per cycle with **0 lacking geometry**, and 6 of the 7 cycles put reversed flow **at the tidal outfall** - 18,375 to 21,392 reversed edges and 2,725 to 8,834 surcharging nodes per cycle. Every earlier run predated ADR-0052's geometry join, so M9's animated dash has drawn nothing since it was built on 2026-09-15. Still unticked: "visible" means seen on screen, and it has not been. The same re-bake put the mass balance over budget - see phase 4.)
...
- [-] P3.7 Tests: spread grows with lead; dry-in dry-out; stage time ≤ 5 s — **the budget is missed and one of section 11.1's three tests fails** (re-measured 2026-09-12). Two of the three tests pass. Timing on this laptop is **6.32 s warm / 8.39 s cold** at the full 20 × 36 × 120 × 120 configuration, not the 5.65 s the board carried; pySTEPS' nowcast is 5.7-6.0 s of it and `num_workers`, the FFT backend and the spectral domain were each measured without beating the default. The test that guards it was named `..._meets_the_five_second_budget` while asserting 2× the budget locally and 4× under CI — honest in its comment, misleading in its name, since a green test called that reads as "budget met". Renamed to say it is a regression guard and to print whether the real budget was met. The third 11.1 test — ensemble mean within 15 % of persistence at lead 0 — had never been written; written now, it measures **median 21.7 % across five storms** (6.1-27.3 %) and ships `xfail(strict=True)`. The first measurement (16.9 %) was taken on a degenerate fixture and its extremes finding did not survive re-measurement on a real field; `probmatching="mean"` would pass it and was rejected for inflating the 99.9th percentile by 52 % (ADR-0040).
- [x] P3.8 Console: radar animation layer and the fan chart at Hindmata read the cube (temporary panel until Phase 6 wires it properly) (2026-09-09, 3724182 — `GET /v1/nowcast/rain` and `/rain/series` under the CLAUDE.md 12 URL shape (ADR-0016); `FanChart` is the section 6.6 component Phase 6 keeps, `SkyPanel` is the scaffolding it deletes. Hindmata's position and its `source_url` come from the city register, never a literal. `run_id` is null on a computed cycle because minting one would claim Twin and Flash versions that never ran)
...
- [-] P4.5 Mass balance assertion < 0.1 % over a 3-hour AOI run - **the assertion is built and the budget is now missed on every shipped cycle** (2026-09-10, 954e968; re-measured 2026-09-23). The check exists, runs every 100 steps and passes its unit tests; what changed is the runs. Re-baking the seven demo cycles measures 0.127, 0.535, 0.145, 0.260, 0.306, 0.204 and 0.310 % against 0.1 %, where the previous bake read 0.014-0.091 % on six of them. This is a **regression**, not a budget that was never met, and it is not diagnosed: these are the first runs to combine ADR-0055's tide datum, ADR-0039's hotspot fix, ADR-0044's ES-MDA update and the reversed-edge geometry join. Marked `[-]` rather than `[x]` because an unqualified tick here would claim the one number that proves the solver conserves water.
...
- [ ] P10.4 Performance pass: bundle analysis, map fps at 1440 × 900, API p95, cycle timings, memory < 1 GB tab; regressions fixed (partial. 2026-09-14: a column-wise segment table makes the segment forecast 8.5 s to 0.97 s for 20 members, bitwise equal (b930b54, ADR-0050). 2026-09-15: it is wired into `depth.py`, the 36 PNGs encode on 8 threads and the wet-segment writer rounds each distinct value once (101a5db, 885edd8, ADR-0051); all 75 depth products of the 08:40 cycle are byte-identical to before and to the shipped bake. Products compute plus writers went from about 10.5-13 s to about 2.7 s warm and 3.6 s cold with 12 falling to 10 python processes, so section 11.8's 2 s is still missed; `stage_ms.products` reads 3.0 s and excludes the writers. Map fps with the M8 pulse on is marginal, 43.7-58.3 on a dev build (ADR-0053))
