...
- [x] P6.12 Ground-truth pins drop on the replay clock with the "As it happened" ticker (2026-09-11 — 29 sourced pins had no endpoint serving them; `GET /v1/replay/ground-truth` does, the map drops each as the clock passes it (M18) and the ticker links the URL each was read from)
...
- [x] P9.1 Landing page sections 1-10 per 7.1, hero embed, OG image, metadata, sitemap (2026-09-10, 505b2a4 - the hero opens on a globe that unrolls into a world map and hands over to the live console map; the topology is committed rather than fetched from a CDN)
...
- [x] P9.3 `/map` public map (mobile-first, three colours, passable-until sheet, vehicle selector, honesty line, saved locations) (2026-09-10)
