...
| 3 | VARUNA-Sky | 90 % | 2026-09-12 | **Not 100 %: the exit criterion is missed and a section 11.1 test fails.** Sky runs QC -> Z-R -> merge -> flow -> STEPS -> products in one `run_sky()` and seven of the eight tasks are done. Re-measured on 2026-09-12: **6.32 s warm / 8.39 s cold** at 20 x 36 on the 120 x 120 domain against the 5 s exit budget (the board previously carried 5.65 s); pySTEPS is 5.7-6.0 s of it and three tuning routes were each measured without beating the default. Ensemble spread still widens from 1.03 to 1.78 mm/h between the 30- and 90-minute leads at Hindmata. **New finding (ADR-0040):** 11.1's third test - ensemble mean within 15 % of persistence at lead 0 - had never been written; it measures **16.9 %**, every member loses the mass individually, and it ships `xfail(strict=True)` rather than with a widened threshold. Earlier fixes hold: the fallback AR(2) ringing to 10,589 mm/h (ADR-0015), the false Z-R fallback label (ADR-0017), and the Twin being fed the ensemble *median* instead of the mean. P2.9 (IMD decoder) stays P1. |
