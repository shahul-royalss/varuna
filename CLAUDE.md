...
- [x] P9.7 `services/verify` per 11.12 and `/verify` page; committed `public/verification.json` (2026-09-10, 27b18fe - CSI 0.22 / POD 0.41 / 31 min median lead at 15 cm, swept across 5/15/30 cm because the pins record waterlogging and not a depth, ADR-0029. The reliability diagram and Brier score are returned as unavailable with their reasons: the baked runs have one member)
...
- [x] P10.9 `docs/QA.md` with every number measured (2026-09-11 — including the ones that are bad: the 8 s budget missed at 74 s, CSI 0.22 at 15 cm, Flash-lite at CSI 0.085, and the 06:40 cycle over its mass-balance budget at 0.18 %)
