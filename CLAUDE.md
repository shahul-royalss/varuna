...
| 9 | Landing, public map, report, onboarding, verify | 70 % | 2026-09-10 | **Landing (P9.1)**: ten sections, opening on a globe that unrolls into a world map and settles on Mumbai before cross-fading into the live console map. `d3-geo`'s projection mutator interpolates the orthographic and equirectangular raw projections, so the coastlines deform correctly rather than cross-fading; the topology is committed to `public/world-110m.json` because CLAUDE.md 17 requires the finale to run with the network off. **Verification (P9.7)**: CSI 0.22, POD 0.41, FAR 0.68 and a 31-minute median lead at 15 cm, swept across 5/15/30 cm because the pins record waterlogging and carry no depth. Every pin in the window is found at 5 cm and two of seventeen at 30 cm - the pattern is right and the level is low, and the screen says so. Depth MAE and the reliability diagram are returned as *unavailable* with their reasons rather than as numbers (ADR-0029). **Public map (P9.3)** draws the run in three colours against the selected vehicle's stopping depth, with passable-until times per street. **Report (P9.4)** stores and queues an observation; the feedback count stays null until a cycle assimilates it. **Outstanding:** Chennai pre-cache and the onboarding wizard (P9.5, P9.6), the API explorer (P9.8, P1), i18n (P9.9, P1) and the PWA (P9.10, P1). |
