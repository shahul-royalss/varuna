...
| 9 | Landing, public map, report, onboarding, verify | 90 % | 2026-09-10 | **Chennai onboards on stage** (P9.5, P9.6). `POST /v1/onboard` runs the city pipeline in a background thread and forwards its own log lines through a structlog processor, so every line the wizard shows came from `services/city`; the six steps on screen are the fourteen pipeline steps folded. **Chennai builds from cache in 36 s** against a 5-minute budget - 18,622 segments, 40,729 inferred pipe edges, 1,854 depressions, 6,147 surface units - and its first forecast delivers 142.6 mm of the design storm's specified 150 mm with a mass balance of 6.0e-04. Five defects had to be fixed before any of that ran, and four were silent: the pre-cache downloaded nothing but DEM tiles; a missing hotspot register and a missing curated infrastructure file each killed the build outright (ADR-0031); `gauge_window` raised on a bundle with no gauges, where Marshall-Palmer is the designed fallback; the Twin refused a coastal domain with no tide series; and an Overpass connection reset was recorded as "this city has no waterways" (ADR-0032, recovered 0 to 54 on retry). **The largest finding is ADR-0030**: a design storm cannot be nowcast. It is spatially uniform by construction and STEPS extrapolates spatial structure, so its twenty members decorrelate into noise and their mean collapses - the AOI received 0.1 mm of a 150 mm storm, and issuing at the storm's peak instead delivered 609 mm. A design-storm bundle now forces the Twin from its own hyetograph and the run says so; `MUM-IDF-25yr` was affected identically. Earlier in the phase: the landing page (P9.1), verification (P9.7), the public map (P9.3) and the report flow (P9.4). **Outstanding:** landing performance (P9.2), the API explorer (P9.8, P1), i18n (P9.9, P1) and the PWA (P9.10, P1). |
| 10 | Polish, rehearsal, packaging | 60 % | 2026-09-11 | **The demo script is a test now** (P10.5): twelve Playwright tests, one per beat of section 15, asserting that the numbers exist rather than what they are - four of them also assert zero console errors, which section 14 requires and nothing was checking. **The accessibility floor is checked rather than assumed** (P10.3): axe over thirteen screens, 0 violations, after fixing four real classes of defect - `--text-3` measured 4.25:1 on `--deep` against the 4.5:1 section 6.10 itself demands (ADR-0036), Base UI's slider thumbs meant **every slider in the app shipped unlabelled**, links in text blocks were distinguished by colour alone, and five scroll regions could not be reached from a keyboard. **`make pack`** writes an 830 MB offline package (P10.6), and the README and `docs/QA.md` carry every measured number including the bad ones (P10.8, P10.9). The **reduced-motion audit** is five more tests (P10.1): with the OS setting emulated the CSS escape hatch flattens everything on the console, and the map canvas is provably still - two screenshots 700 ms apart compare byte-identical, which is the only way to observe a WebGL pulse. 35 e2e tests in total. **Outstanding:** the motion catalogue's own gaps (M9 reversed-flow dash, M15 isochrone morph, M19 onboarding layer stack), the per-screen design QA and screenshots (P10.2), the performance pass (P10.4), the fallback video (P10.7) and the two rehearsals (P10.10). |
